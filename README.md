...
5. **智能磁盘容量保护与自动淘汰（最多保留 40 首）**：
   * 为了防止长时间点歌将硬盘空间占满，服务内置了自动淘汰清理机制（默认 `MAX_CACHED_SONGS = 40`）。
   * 每当一首新歌完整下载并写入磁盘后（以及服务每次启动时），自动扫描 `downloads/` 目录；
   * 当歌曲总数超过 40 首时，系统按照下载时间先后顺序自动删除**最早下载的一首或多首歌曲**，确保硬盘中最多保留 40 首音乐；
   * 同时自动回收网络中断或异常遗留的 `.tmp` 临时分片文件。
